...
// This is equivalent to derive(Default), except the impl generated by derive is overly restrictive
// and requires that all types parameters conform to Default, which is not needed at all.
...
{
    fn default() -> Self {
        DiGraph { nodes: Default::default(), next_ord: Default::default() }
    }
}
...
mod tests {
    ...
    fn test_no_self_cycle() {
        ...
        assert_eq!(graph.add_edge(1, 1, ()), Err(vec![(1, ())]));
    }
    ...
    fn test_digraph() {
        ...
        // Add some safe edges
        assert!(graph.add_edge(0, 1, "a").is_ok());
        assert!(graph.add_edge(1, 2, "b").is_ok());
        assert!(graph.add_edge(2, 3, "c").is_ok());
        assert!(graph.add_edge(4, 2, "d").is_ok());

        // Try to add an edge that introduces a cycle
        assert_eq!(graph.add_edge(3, 1, "e"), Err(vec![(3, "c"), (2, "b")]));

        // Add an edge that should reorder 0 to be after 4
        assert!(graph.add_edge(4, 0, "f").is_ok());
    }
